namespace MochaDB.mhql.keywords {
  ...
  internal class Mhql_DELCOL:MhqlKeyword {
    ...
    public string GetDELCOL(string command,out string final) {
      ...
      if(groupbydex==-1)
        throw new InvalidOperationException("DELCOL command is cannot processed!");
    ...
    public void Delcol(string command,ref MochaTableResult table) {
      ...
      if(parts.Length > 2)
        throw new ArgumentOutOfRangeException("The DELCOL command can take up to 2 parameters!");
      if(parts.Length == 1) {
        int count;
        if(!int.TryParse(command,out count))
          throw new ArithmeticException("The parameter of the DELCOL command was not a number!");
        if(count < 1)
          throw new ArgumentOutOfRangeException("The parameters of the DELCOL command cannot be less than 1!");
        ...
        if(start < 1 || count < 1)
          throw new ArgumentOutOfRangeException("The parameters of the DELCOL command cannot be less than 1!");
        IEnumerable<MochaColumn> deleted = table.Columns.Skip(start-1).Take(count);
